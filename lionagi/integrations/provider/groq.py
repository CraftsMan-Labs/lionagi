...
class GroqService(BaseService):
    """
    A service to interact with Groq's API endpoints.

    Attributes:
        base_url (str): The base URL for the Groq API.
        available_endpoints (list): A list of available API endpoints.
        schema (dict): The schema configuration for the API.
        key_scheme (str): The environment variable name for Groq API key.
